...
function Chat({ addToast }) {
  ...
  // Load all consultant chat messages grouped by customer
  useEffect(()=>{
    const unsub = onSnapshot(
      ...
      snap=>{
        ...
        const byCustomer = {};
        msgs.forEach(m=>{
          const key = m.name||'Анонимный';
          if(!byCustomer[key]) byCustomer[key]={name:key,messages:[],lastTime:m.time,unread:0};
          byCustomer[key].messages.push(m);
          if(m.time>byCustomer[key].lastTime) byCustomer[key].lastTime=m.time;
          if(m.from!=='admin'&&!m.read) byCustomer[key].unread++;
        });
        setThreads(Object.values(byCustomer).sort((a,b)=>b.lastTime.localeCompare(a.lastTime)));
      }
    );
    return()=>unsub();
  },[]);
  ...
  useEffect(()=>{
    if(!active) return;
    const thread = threads.find(t=>t.name===active);
    if(thread){ setMessages(thread.messages); setTimeout(()=>bottomRef.current?.scrollIntoView({behavior:'smooth'}),100); }
  },[active, threads]);

  const send = async () => {
    if(!reply.trim()) return;
    try {
      await addDoc(collection(db,'consultant_chat'),{ from:'admin', name:'Queen Star', text:reply.trim(), time:nowStr(), read:true });
      setReply('');
    } catch { addToast('Ошибка отправки', RED); }
  };
    ...
    return(
      <div style={{ position:'fixed', inset:0, background:BG, display:'flex', flexDirection:'column' }}>
        <div style={{ background:CARD, borderBottom:`1px solid ${LINE}`, padding:'13px 16px', display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div style={{ width:34, height:34, borderRadius:'50%', background:GOLD, display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, color:'#fff', fontWeight:700 }}>{active[0]?.toUpperCase()}</div>
          ...
          {messages.map(m=>(
            <div key={m.id} style={{ display:'flex', justifyContent:m.from==='admin'?'flex-end':'flex-start' }}>
              <div style={{ maxWidth:'80%', background:m.from==='admin'?GOLD:CARD2, color:'#fff', borderRadius:m.from==='admin'?'14px 4px 14px 14px':'4px 14px 14px 14px', padding:'10px 13px', fontSize:13 }}>
                {m.text}
                <div style={{ fontSize:9, opacity:0.7, marginTop:4 }}>{new Date(m.time).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}</div>
              </div>
            </div>
          ))}
          <div ref={bottomRef}/>
        ...
        <div style={{ background:CARD, borderTop:`1px solid ${LINE}`, padding:'10px 14px', display:'flex', gap:8 }}>
          <input style={{ ...S.input, flex:1, padding:'10px 12px' }} value={reply} onChange={e=>setReply(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')send();}} placeholder="Написать ответ..."/>
  ...
  return(
    <div style={{ padding:'14px 14px 80px' }}>
      <div style={{ fontSize:18, fontWeight:700, color:INK, marginBottom:14 }}>💬 Чат с покупателями {totalUnread>0&&<span style={{color:RED,fontSize:13}}>({totalUnread} новых)</span>}</div>
      {threads.length===0&&<div style={{ textAlign:'center', padding:'40px 0', color:MUTED }}>Сообщений пока нет</div>}
      {threads.map(t=>{
        const last=t.messages[t.messages.length-1];
        return(
          <div key={t.name} onClick={()=>setActive(t.name)} style={{ ...S.card, padding:14, marginBottom:9, display:'flex', alignItems:'center', gap:12, cursor:'pointer' }}>
            <div style={{ width:44, height:44, borderRadius:'50%', background:t.unread>0?RED:CARD2, display:'flex', alignItems:'center', justifyContent:'center', fontSize:18, color:'#fff', fontWeight:700, flexShrink:0 }}>
              {t.unread>0?t.unread:t.name[0]?.toUpperCase()}
            </div>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontSize:14, fontWeight:700, color:INK }}>{t.name}</div>
              <div style={{ fontSize:12, color:MUTED, marginTop:2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{last?.text}</div>
            </div>
            <div style={{ fontSize:10, color:MUTED, flexShrink:0 }}>{last&&new Date(last.time).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}</div>
          </div>
        );
      })}
    </div>

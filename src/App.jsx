  ...
  const send = async () => {
    ...
    try {
      await addDoc(collection(db,'consultant_chat'),{
        ...
        text:reply.trim(), time:nowStr(), read:true
      });
      // Mark customer messages as read
      const unread = messages.filter(m=>m.name===active&&m.from!=='admin'&&!m.read);
      await Promise.all(unread.map(m=>updateDoc(doc(db,'consultant_chat',m.id),{read:true}).catch(()=>{})));
      setReply('');
    } catch(e){ addToast('Ошибка отправки', RED); }
  };
      ...
      <div style={{ position:'fixed', inset:0, background:BG, display:'flex', flexDirection:'column' }}>
        <div style={{ background:CARD, borderBottom:`1px solid ${LINE}`, padding:'13px 16px', display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:11, color:GREEN }}>● онлайн</div>
          </div>
        </div>
        ...
        <div style={{ background:CARD, borderTop:`1px solid ${LINE}`, padding:'10px 14px', display:'flex', gap:8 }}>
          <input style={{ ...S.input, flex:1, padding:'11px 13px' }}
            ...
            placeholder={`Ответить ${active}...`} autoFocus/>
          <button style={{ ...S.btn(GOLD), width:'auto', padding:'11px 18px' }} onClick={send}>→</button>
        </div>
      </div>
    );
  }
